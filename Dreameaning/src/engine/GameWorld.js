...
export class GameWorld {
    ...
    update() {
        if (this.transitioning) {
            this.transitionProgress += 0.025; // Slower transition (takes ~700ms)
    ...
    draw() {
        ...
        if (this.transitioning) {
            const ease = 1 - Math.pow(1 - this.transitionProgress, 3); // ease-out
            // 정확한 방향으로 방이 슬라이드해 들어오도록 (-) 부호 적용
            camX = -this.transitionDir.x * (1 - ease);
            camY = -this.transitionDir.y * (1 - ease);
        }
        ...
        if (window.canInteractWithComputer && !this.player.interacting && !this.transitioning) {
            ...
            ctx.fillText('컴퓨터와 상호작용 (E)', this.renderer.width / 2, this.renderer.height / 2 - 80);
        }
    }
    ...
    handleWallCollisions() {
        ...
        if (a < 0 || a > 1 || b < 0 || b > 1) {
            ...
            if (dirX !== 0 || dirY !== 0) {
                ...
                this.player.x = fixed.x - (dirX / Math.hypot(dirX, dirY)) * 10;
                this.player.y = fixed.y - (dirY / Math.hypot(dirX, dirY)) * 10;
            } else {
